...
public class DinergateMercRenderer extends MobRenderer<DinergateMercEntity, ModelDinergate<DinergateMercEntity>> {
	public DinergateMercRenderer(EntityRendererProvider.Context context) {
		super(context, new AnimatedModel(context.bakeLayer(ModelDinergate.LAYER_LOCATION)), 0.5f);
	}

	@Override
	protected void scale(DinergateMercEntity entity, PoseStack poseStack, float f) {
		poseStack.scale(1.5f, 1.5f, 1.5f);
	}

	@Override
	public ResourceLocation getTextureLocation(DinergateMercEntity entity) {
		return ResourceLocation.parse("gflmod:textures/entities/dinergatemerc.png");
	}

	private static final class AnimatedModel extends ModelDinergate<DinergateMercEntity> {
		private final ModelPart root;
		private final HierarchicalModel animator = new HierarchicalModel<DinergateMercEntity>() {
			@Override
			...
			@Override
			public void setupAnim(DinergateMercEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
