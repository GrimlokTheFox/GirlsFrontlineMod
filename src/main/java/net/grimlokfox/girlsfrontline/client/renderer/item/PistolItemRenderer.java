package net.grimlokfox.girlsfrontline.client.renderer.item;
...
@OnlyIn(Dist.CLIENT)
public class PistolItemRenderer extends BlockEntityWithoutLevelRenderer {
	private final EntityModelSet entityModelSet;
	private final ItemStack transformSource;

	public PistolItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher, EntityModelSet entityModelSet) {
		super(blockEntityRenderDispatcher, entityModelSet);
		this.entityModelSet = entityModelSet;
		this.transformSource = new ItemStack(GflmodModItems.PISTOL.get());
	}
	...
	public void renderByItem(ItemStack itemstack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
		Model model = new ModelPistol(this.entityModelSet.bakeLayer(ModelPistol.LAYER_LOCATION));
		ResourceLocation texture = ResourceLocation.parse("gflmod:textures/item/texture.png");
		if (model == null)
			return;
		poseStack.pushPose();
